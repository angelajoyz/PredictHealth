...
const getConfidence = (disease) =>
  ({ dengue_cases:87, diarrhea_cases:82, respiratory_cases:79,
     ...
     hypertension_cases:83, diabetes_cases:80 })[disease] ?? 78;
...
// ── Dashboard ──────────────────────────────────────────────────────────────────
// ✅ FIXED: Hindi na nag-require ng uploadedFile — data comes from database
const Dashboard = ({ onNavigate, onLogout }) => {
  ...
  // ✅ hasDbData — true kung may nakaload na barangays mula sa previous upload
  ...
  useEffect(() => {
    try {
      ...
      if (bar) {
        const p = JSON.parse(bar);
        if (Array.isArray(p) && p.length > 0) {
          setAvailableBarangays(p);
          setHasDbData(true); // ✅ May data na sa DB
        }
      }
  ...
  const computeInsights = (history) => {
    ...
    setTotalForecasted(
      history.filter(h => h.period === latest).reduce((s, h) => s + (h.predictedValue || 0), 0)
    );
    const dMap = {};
  ...
  const buildTrendSummary = () => {
    ...
    filtered.forEach(d => {
      ...
      if (d.trend === 'increasing')
        items.push({ type: 'increasing', text: `${d.label} cases are projected to increase${d.pct ? ' by ' + d.pct : ''} in ${loc} — increased monitoring recommended.` });
      else if (d.trend === 'decreasing')
        items.push({ type: 'decreasing', text: `${d.label} cases are projected to decline${d.pct ? ' by ' + d.pct : ''} in ${loc} — positive outlook.` });
      else
        items.push({ type: 'stable', text: `${d.label} cases are expected to remain stable in ${loc}.` });
    });
  ...
  // ── ✅ FIXED: Generate Forecast — reads from DB, no file needed ───────────
  const handleGenerateForecast = async () => {
    // Check kung may data sa DB (availableBarangays from localStorage = uploaded na dati)
    if (!hasDbData || availableBarangays.length === 0) {
      setForecastError('No dataset found in the database. Please go to Data Import and upload your health data first.');
      return;
    }
    if (availableDiseases.length === 0) {
      setForecastError('No disease columns found. Please re-upload your dataset in Data Import.');
      return;
    }
    ...
    try {
      ...
      // ✅ Direktang i-send ang barangay sa backend — mag-kukunin ng data sa DB
      const result = await getForecast(selectedBarangay, diseases, months, city);

      setForecastData(result);
      setDisplayedBarangay(selectedBarangay);
      setDisplayedHorizon(forecastHorizon);

      // Update available diseases mula sa result
      ...
      const newEntries = Object.keys(result.predictions).flatMap((disease, idx) => {
        ...
        return result.forecast_dates.map((fd, i) => ({
          ...
          status:         'Completed',
          createdAt:      dateStr,
          fileName:       uploadedInfo?.fileName || 'dataset',
  ...
  const isStale    = forecastData && displayedBarangay &&
                     (displayedBarangay !== selectedBarangay || displayedHorizon !== forecastHorizon);
  const isUpToDate = forecastData && displayedBarangay &&
                     displayedBarangay === selectedBarangay && displayedHorizon === forecastHorizon;
  ...
  const totalThisMonth = forecastHistory
    .filter(h => h.period === thisMonthKey)
    .reduce((s, h) => s + (h.predictedValue || 0), 0);
        ...
        <Box sx={{ px: '24px', pt: '20px', pb: '28px', overflow: 'auto', flex: 1 }}>
          ...
          {!hasDbData && (
            <Alert severity="info" icon={<InfoOutlinedIcon fontSize="small" />}
              sx={{ mb: 2.5, borderRadius: '10px', backgroundColor: T.blueDim,
                color: T.textHead, border: `1px solid rgba(37,99,235,0.18)`,
                fontSize: 13, '& .MuiAlert-icon': { color: T.blue } }}>
            ...
            <CardContent sx={{ p: '14px 16px', '&:last-child': { pb: '14px' } }}>
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Select value={selectedBarangay} size="small" sx={{ ...SelectSx, minWidth: 150 }}
                    displayEmpty onChange={(e) => setSelectedBarangay(e.target.value)}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Select value={selectedDisease} size="small" sx={{ ...SelectSx, minWidth: 140 }}
                    ...
                    {availableDiseases.map(col => (
                      <MenuItemComponent key={col} value={col} sx={{ fontSize: 13 }}>
                        {getDiseaseInfo(col).label}
                      </MenuItemComponent>
                    ))}
                  </Select>
                </Box>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Select value={forecastHorizon} size="small" sx={{ ...SelectSx, minWidth: 130 }}
                ...
                {/* ✅ FIXED: disabled kapag walang data sa DB — hindi na kailangan ng uploadedFile */}
                <Button variant="contained" onClick={handleGenerateForecast}
                  disabled={forecastLoading || !hasDbData}
                  startIcon={forecastLoading
                    ? <CircularProgress size={13} color="inherit" />
                    : <PsychologyIcon sx={{ fontSize: 15 }} />}
                  sx={{ ml: 'auto', textTransform: 'none', fontWeight: 600, fontSize: 13,
          ...
          {isStale && (
            <Box sx={{ display: 'flex', alignItems: 'center', px: 2, py: 1.25, mb: '14px',
            ...
            <SCard sx={{ borderTop: `3px solid ${T.blue}` }}>
              <CardContent sx={{ p: '16px', '&:last-child': { pb: '16px' } }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.25 }}>
                  ...
                  <Box sx={{ width: 28, height: 28, borderRadius: '6px', backgroundColor: T.blueDim,
              ...
              <CardContent sx={{ p: '16px', '&:last-child': { pb: '16px' } }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.25 }}>
                  ...
                  <Box sx={{ width: 28, height: 28, borderRadius: '6px',
                    backgroundColor: trendBg(stats?.trend || 'stable'),
                    display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <Typography sx={{ fontSize: 10.5, color: T.textFaint, mt: 0.25 }}>
                  {stats?.nextMonth || nextMonthLabel}
                </Typography>
              </CardContent>
            </SCard>
            ...
            <SCard sx={{ borderTop: `3px solid ${T.ok}` }}>
              <CardContent sx={{ p: '16px', '&:last-child': { pb: '16px' } }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.25 }}>
                  ...
                  <Box sx={{ width: 28, height: 28, borderRadius: '6px', backgroundColor: T.okBg,
                    display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <Typography sx={{ fontSize: 26, fontWeight: 700, color: T.textHead, lineHeight: 1, letterSpacing: '-0.5px' }}>
                  ...
                  <Typography component="span" sx={{ fontSize: 14, fontWeight: 600, color: T.textMuted }}>
                    {parseInt(forecastHorizon) === 1 ? 'Month' : 'Months'}
                  </Typography>
                </Typography>
                <Typography sx={{ fontSize: 10.5, color: T.textFaint, mt: 0.5 }}>
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={chartData} margin={{ top: 8, right: 12, left: -14, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="actual" name="Actual" stroke={T.ok} strokeWidth={2}
                      dot={{ fill: T.ok, r: 3, strokeWidth: 0 }}
                      activeDot={{ r: 4, fill: T.ok, stroke: '#fff', strokeWidth: 2 }}
                      connectNulls={false} />
            ...
            <CardContent sx={{ p: '18px 20px', '&:last-child': { pb: '20px' } }}>
              ...
              <Box sx={{ mt: 2, display: 'flex', gap: 2 }}>
                <Button size="small" onClick={() => onNavigate?.('prediction')}
                  sx={{ textTransform: 'none', color: T.blue, fontSize: 12, fontWeight: 500,
                    px: 0, '&:hover': { backgroundColor: 'transparent', opacity: 0.7 } }}>
                  View detailed predictions →
                </Button>
                <Button size="small" onClick={() => onNavigate?.('history')}
                  sx={{ textTransform: 'none', color: T.textMuted, fontSize: 12, fontWeight: 500,
                    px: 0, '&:hover': { backgroundColor: 'transparent', opacity: 0.7 } }}>
                  View history
                </Button>
              </Box>
