...
const Prediction = ({ onNavigate, onLogout, uploadedFile, uploadedData }) => {
  ...
  useEffect(() => {
    ...
    // ✅ NEW: Load forecast history from localStorage
    const savedHistory = localStorage.getItem('forecastHistory');
    if (savedHistory) {
      try {
        setForecastHistory(JSON.parse(savedHistory));
      } catch (e) {
        console.error('Error loading forecast history:', e);
      }
    }
  }, [uploadedData, uploadedFile]);

  // ✅ NEW: Save forecast history to localStorage whenever it changes
  useEffect(() => {
  ...
  // ✅ NEW: Save dashboard snapshot data when forecast completes (not on every disease change)
  useEffect(() => {
    if (forecastData && selectedBarangay && forecastData.historical_data && forecastData.historical_data.dates && !forecastLoading) {
      // Save snapshot for the currently selected view
      ...
      if (stats && miniTrend.length > 0) {
        const dashboardSnapshot = {
          ...
          totalCases: stats.nextVal,
          trendDirection: stats.trend,
          trendPercentage: stats.pct,
          ...
          miniTrendData: miniTrend,
          lastUpdated: new Date().toISOString(),
        };
        
        console.log('💾 Saving dashboard snapshot for:', selectedDisease);
        localStorage.setItem('dashboardSnapshot', JSON.stringify(dashboardSnapshot));
      } else {
        console.log('⚠️ Cannot save dashboard snapshot - missing stats or trend data');
      }
    }
  }, [forecastData, forecastLoading]);

  // ✅ NEW: Build mini trend data for dashboard
  const buildMiniTrendData = () => {
    if (!forecastData || !forecastData.historical_data || !forecastData.historical_data.dates) {
      console.log('⚠️ No forecast data available for mini trend');
      return [];
    }
    
    // Always get last 6 months from historical data
    const histDates = forecastData.historical_data.dates.slice(-6);
    
    if (histDates.length === 0) {
      console.log('⚠️ No historical dates available');
      return [];
    }
    
    const trendData = histDates.map((date, i) => {
      let totalCases = 0;
      
      if (selectedDisease === 'all') {
        // Sum all diseases for this month
        Object.keys(forecastData.predictions).forEach(disease => {
          const values = forecastData.historical_data[disease] || [];
          const slicedValues = values.slice(-6);
          totalCases += slicedValues[i] || 0;
        });
      } else {
        // Get single disease value
        const values = forecastData.historical_data[selectedDisease] || [];
        const slicedValues = values.slice(-6);
        totalCases = slicedValues[i] || 0;
      }
      
      return {
        month: date.slice(5, 7), // Get MM from YYYY-MM
        cases: Math.round(totalCases),
      };
    });
    
    console.log('📊 Mini trend data built:', trendData);
    return trendData;
  };
  ...
  // ── AUTO-GENERATE FORECAST when component loads (ONCE) ───────────────────────────
  ...
  const buildChartData = () => {
    if (!forecastData) {
      console.log('⚠️ No forecast data for chart');
      return [];
    }
    
    if (!forecastData.historical_data || !forecastData.historical_data.dates) {
    ...
    if (selectedDisease === 'all') {
      ...
      histDates.forEach((date, i) => {
        ...
        activeDiseases.forEach(disease => {
          const values = forecastData.historical_data[disease] || [];
          const slicedValues = values.slice(-9);
          totalActual += slicedValues[i] || 0;
        });
      ...
      if (forecastData.forecast_dates && forecastData.forecast_dates.length > 0) {
      ...
      if (!disease) {
        console.log('⚠️ No disease selected');
        return [];
      }

      // Check if this disease exists in the forecast data
      if (!forecastData.historical_data[disease]) {
        console.log(`⚠️ Disease ${disease} not found in historical data`);
        return [];
      }
      ...
      if (histDates.length === 0) {
        console.log('⚠️ No historical dates for single disease');
        return [];
      }
      ...
      // Forecast months (predicted data only)
      if (forecastData.forecast_dates && forecastData.forecast_dates.length > 0 && forecastData.predictions[disease]) {
        forecastData.forecast_dates.forEach((date, i) => {
          data.push({
            month: date.slice(0, 7),
            actual: null,
    ...
    console.log('📊 Chart data built:', data.length, 'points'); // ✅ DEBUG: Check the dates
    return data;
  };
  ...
  const handleGenerateForecast = async (overrideHorizon) => {
    ...
    try {
      ...
      // ✅ FIX: use override horizon if provided
      ...
      const newEntries = Object.keys(result.predictions)
        .map((disease, idx) => {
      ...
      setForecastError(
        err.message || 'Forecast failed. Please try again.'
      );
    } finally {
  ...
  // ✅ DEBUG: Log when disease selection changes
  useEffect(() => {
    console.log('🔄 Disease changed to:', selectedDisease);
    console.log('Current forecast data exists:', !!forecastData);
    if (forecastData) {
      console.log('Available diseases in forecast:', Object.keys(forecastData.predictions || {}));
      console.log('Active diseases:', activeDiseases);
    }
  }, [selectedDisease]);

  const stats = React.useMemo(() => {
    const result = getSummaryStats();
    console.log('📊 Stats calculated for', selectedDisease, ':', result);
    return result;
  }, [forecastData, selectedDisease, activeDiseases]);

  const chartData = React.useMemo(() => {
    const result = buildChartData();
    console.log('📈 Chart data calculated for', selectedDisease, ':', result?.length, 'points');
    return result;
  }, [forecastData, selectedDisease, activeDiseases]);

  const insights = React.useMemo(() => {
    return getKeyInsights();
  }, [forecastData, selectedBarangay]);
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            ...
            <Select 
              ...
              onChange={(e) => {
                ...
                if (uploadedFile && selectedBarangay) {
                  handleGenerateForecast(newHorizon);
                }
              }}
              size="small"
              sx={{ minWidth: 180, backgroundColor: 'white', borderRadius: 2 }}
            >
