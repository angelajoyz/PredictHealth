...
const Prediction = ({ onNavigate, onLogout, uploadedFile, uploadedData }) => {
  ...
  // ── AUTO-GENERATE FORECAST when component loads ───────────────────────────
  useEffect(() => {
    if (uploadedFile && selectedBarangay && availableDiseases.length > 0 && !forecastData && !forecastLoading) {
  ...
  const handleGenerateForecast = async () => {
    ...
    try {
      ...
      const months = parseInt(forecastHorizon) || 3;
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            ...
            <Select 
              ...
              onChange={(e) => {
                ...
                // Auto-regenerate forecast when horizon changes
                setTimeout(() => {
