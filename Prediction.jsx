...
const Prediction = ({ onNavigate, onLogout, uploadedFile, uploadedData }) => {
  ...
  // ── Load barangay + disease columns + history on mount ────────────────────
  useEffect(() => {
  ...
  // ✅ Save forecast history to localStorage whenever it changes
  useEffect(() => {
  ...
  // ✅ Save dashboard snapshot when forecast completes
  useEffect(() => {
  ...
  // ✅ Build mini trend data for dashboard
  ...
  const handleGenerateForecast = async (overrideHorizon, overrideDisease) => {
    ...
    setForecastError('');
    // ✅ DON'T clear forecastData - keep old data visible while loading

    try {
      // ✅ Use overrideDisease if provided (for forcing "all")
      const diseaseSelection = overrideDisease !== undefined ? overrideDisease : selectedDisease;
      
      const diseases =
        diseaseSelection === 'all'
          ? availableDiseases
          : [diseaseSelection];
      ...
      // ✅ Clear horizon change message after successful generation
  ...
  return (
    <Box className="dashboard-container">
      ...
      <Box className="main-content">
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            ...
            <Select 
              ...
              onChange={(e) => {
                ...
                const wasSpecificDisease = selectedDisease !== 'all';
                
                setForecastHorizon(newHorizon);

                // ✅ Show message and switch UI to "All Diseases"
                if (wasSpecificDisease) {
                ...
                if (uploadedFile && selectedBarangay) {
                  // ✅ CRITICAL: Pass 'all' explicitly to force generation of all diseases
                  handleGenerateForecast(newHorizon, 'all');
                }
              }}
              size="small"
        ...
        {/* ✅ Show info when auto-switching to All Diseases */}
        {horizonChangeMessage && (
          <Alert severity="info" sx={{ mb: 2 }} onClose={() => setHorizonChangeMessage('')}>
            {horizonChangeMessage}
          </Alert>
        )}
